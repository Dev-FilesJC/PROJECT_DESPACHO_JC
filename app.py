...
if st.session_state.page == "cajas":
    ...
    if es_admin():
        # Control explícito del formulario de ingreso de stock.
        # Así CANCELAR puede ocultarlo de forma confiable.
        st.session_state.setdefault("mostrar_ingreso_stock", False)
        st.session_state.setdefault("stock_form_version", 0)

        if st.session_state.mostrar_ingreso_stock:
            with st.expander("➕ INGRESAR LLEGADA DE STOCK", expanded=True):
                ...
                if cancelar_stock:
                    # No guarda nada: limpia y oculta completamente el formulario.
            ...
            # El expander cerrado se muestra como antes.
            with st.expander("➕ INGRESAR LLEGADA DE STOCK", expanded=False):
                if st.button(
                    "➕ ABRIR FORMULARIO DE INGRESO DE STOCK",
                    key="abrir_formulario_stock",
                    use_container_width=True
                ):
                    st.session_state.mostrar_ingreso_stock = True
                    st.rerun()
    else:
