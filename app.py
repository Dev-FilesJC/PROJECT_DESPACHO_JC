...
def descargar_excel_github(destino=CLOUD_PATH):
    """Descarga el XLSX real desde GitHub y valida que no esté corrupto."""
    ...
    # Para archivos grandes GitHub puede no devolver `content`; en ese caso
    # usamos download_url. Siempre validamos que el resultado sea un XLSX.
    content = "".join(str(data.get("content", "")).split())
    if content:
        try:
            raw = base64.b64decode(content, validate=False)
            if raw and zipfile.is_zipfile(BytesIO(raw)):
    ...
    if not download_url:
        raise RuntimeError(
            "GitHub no devolvió contenido ni una URL de descarga para el Excel."
        )
    ...
    if not zipfile.is_zipfile(BytesIO(raw)):
        # Evita dejar un archivo inválido en /tmp.
        destino.unlink(missing_ok=True)
        preview = raw[:120].decode("utf-8", errors="replace").replace("\n", " ")
        raise RuntimeError(
            "El archivo recibido desde GitHub no es un XLSX válido. "
            f"Inicio recibido: {preview!r}. Verifica que "
            f"'{GITHUB_EXCEL}' sea realmente un archivo .xlsx y no un archivo "
            "LFS/puntero de texto."
        )
