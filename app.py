...
# Si un navegador se cierra, pierde Internet o queda abandonado, el bloqueo
# no puede quedar activo para siempre. Se libera después de 15 minutos
# sin ninguna interacción de la sesión propietaria.
LOCK_TIMEOUT_SECONDS = 15 * 60


def _limpiar_bloqueo_vencido():
    with CLOUD["lock"]:
        owner = CLOUD.get("owner_id")
        last_activity = CLOUD.get("last_activity")
        if owner and last_activity:
            if time.time() - last_activity > LOCK_TIMEOUT_SECONDS:
                CLOUD["owner_id"] = None
                CLOUD["usuario"] = None
                CLOUD["hora_inicio"] = None
                CLOUD["motivo"] = None
                CLOUD["last_activity"] = None
...
def leer_bloqueo_edicion(ruta):
    ...
    with CLOUD["lock"]:
        ...
        return {
            ...
            "timestamp": CLOUD.get("last_activity") or time.time(),
...
# =========================================================
# PANTALLA DE INICIO DESPUÉS DE SALIR
# =========================================================
if st.session_state.logged_out:
    st.markdown(
        """
        <div style="max-width:760px;margin:90px auto 20px auto;text-align:center;">
            <div style="font-size:64px;">📝</div>
            <h1 style="margin-bottom:10px;">JC CONTROL DE SOLICITUDES</h1>
            <p style="font-size:20px;opacity:.75;">DESPACHO</p>
            <p style="font-size:16px;opacity:.65;margin-top:25px;">
                Has salido correctamente del sistema.
            </p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    _, inicio_col, _ = st.columns([2, 1, 2])
    with inicio_col:
        if st.button("🚪 INGRESAR AL SISTEMA", use_container_width=True, key="volver_inicio_sistema"):
            st.session_state.logged_out = False
...
with u1:
    st.session_state.usuario_nombre = st.text_input(
        "Usuario", value=st.session_state.usuario_nombre,
        placeholder="Ej.: Juan", key="nombre_usuario_cloud"
    ).strip()
with u2:
    st.markdown(
...
if not st.session_state.usuario_nombre:
    st.info("👤 Escribe tu nombre de usuario para que el sistema pueda mostrar quién tiene el bloqueo.")
...
if st.session_state.page == "rutas":
    ...
    if editing is None and not st.session_state.registro_activo:
        if (
            ...
            st.warning(
                f"🔒 **ARCHIVO EN USO — {usuario} está registrando o editando una solicitud"
            ...
            if st.button("🔐 INICIAR NUEVA SOLICITUD", use_container_width=False):
                if not st.session_state.usuario_nombre:
                    st.warning("👤 Primero escribe tu nombre de usuario.")
                    st.stop()
                ...
                if ok:
                    ...
                    st.warning(
                        f"🔒 **{usuario} está registrando una solicitud.** "
    ...
    if editing is not None:
        ...
        if not ok:
            ...
            st.warning(
                f"🔒 **No puedes editar todavía. {usuario} está utilizando el archivo.** "
                f"{'Equipo: ' + equipo + '. ' if equipo else ''}"
                "Espera a que termine."
            )
    ...
    # Mantener vivo el bloqueo mientras el usuario interactúa con el formulario.
    if st.session_state.get("registro_activo"):
        renovar_bloqueo_edicion(st.session_state.excel_path)
