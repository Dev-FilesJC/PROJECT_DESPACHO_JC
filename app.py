...
def descargar_excel_github(destino=CLOUD_PATH):
    """Descarga el Excel directamente desde GitHub y valida el XLSX."""
    destino = Path(destino)

    # 1) Intentar la API de GitHub en modo RAW. Esto evita problemas
    #    con el campo `content` de la API cuando el archivo es grande.
    raw_api_url = (
        f"{GITHUB_API}/repos/{GITHUB_OWNER}/{GITHUB_REPO}"
        f"/contents/{GITHUB_EXCEL}"
    )

    headers_raw = github_headers().copy()
    headers_raw["Accept"] = "application/vnd.github.raw+json"
    ...
    # Si GitHub devolvió JSON en lugar del archivo RAW, usamos la API normal.
    ...
    if "application/json" in content_type:
        ...
        content = "".join(str(data.get("content", "")).split())
        if content:
            try:
    ...
    # 2) Validación definitiva: XLSX es un ZIP válido.
    if not raw or not zipfile.is_zipfile(BytesIO(raw)):
        destino.unlink(missing_ok=True)
        preview = raw[:120].decode("utf-8", errors="replace").replace("\n", " ")
        raise RuntimeError(
            "GitHub respondió, pero el archivo no es un XLSX válido. "
            f"Inicio recibido: {preview!r}. "
            f"Verifica que '{GITHUB_EXCEL}' sea un Excel .xlsx real y no "
            "un archivo LFS/puntero o un archivo dañado."
        )

    destino.write_bytes(raw)

    # Obtener SHA para futuras actualizaciones.
    r_sha = requests.get(
        github_url(),
...
def mostrar_login():
    st.markdown(
        """
        <div style="text-align:center;margin:8px auto 6px auto;">
            <div style="font-size:28px;line-height:1;">🔐</div>
            <div style="font-size:21px;font-weight:800;margin-top:4px;">
                Iniciar sesión
            </div>
            <div style="font-size:11px;opacity:.60;margin-top:2px;">
...
if st.session_state.page == "rutas":
    ...
    if editing is None and not st.session_state.registro_activo:
        if (
            ...
            st.warning(
                f"🔒 **ARCHIVO EN USO — {usuario} está "
                f"{'editando una solicitud' if bloqueo_actual.get('motivo') == 'edición de solicitud' else 'ingresando una solicitud'}.** "
    ...
    if guardar:
        ...
        if not numero_norm or not all(
            ...
            try:
                # IMPORTANTE:
                # Antes de guardar se descarga la versión más reciente de GitHub.
                # Así no se sobrescriben registros agregados por otro usuario.
                descargar_excel_github(CLOUD_PATH)
                st.session_state.excel_path = str(CLOUD_PATH)
    ...
    with d1:
        if st.button("💾 GUARDAR / ACTUALIZAR EXCEL", use_container_width=True):
            try:
                # Sincroniza primero la copia local con GitHub para evitar
                # publicar una versión antigua del Excel.
                descargar_excel_github(CLOUD_PATH)
                st.session_state.excel_path = str(CLOUD_PATH)
                rows_actuales, rutas_actuales = cargar_excel_desde_ruta(CLOUD_PATH)
                st.session_state.rows = rows_actuales
                st.session_state.rutas = rutas_actuales
                guardar_excel_directo(
                    CLOUD_PATH,
    ...
    # Descarga de los resultados filtrados
