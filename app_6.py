...
def _validar_xlsx(ruta):
    """Comprueba que el archivo sea un XLSX ZIP válido y que openpyxl pueda abrirlo."""
    ruta = Path(ruta)
    if not ruta.exists() or ruta.stat().st_size < 100:
        raise RuntimeError("El archivo XLSX está vacío o incompleto.")

    # Un .xlsx es un ZIP. Además comprobamos que no esté truncado.
    if not zipfile.is_zipfile(ruta):
        inicio = ruta.read_bytes()[:80]
        raise RuntimeError(
            "El archivo recibido no es un XLSX válido. "
            f"Inicio recibido: {inicio!r}"
        )
    ...
    try:
        with zipfile.ZipFile(ruta, "r") as zf:
            malo = zf.testzip()
            if malo:
                raise RuntimeError(f"El XLSX está dañado en el componente: {malo}")
            nombres = set(zf.namelist())
            if "[Content_Types].xml" not in nombres or "xl/workbook.xml" not in nombres:
                raise RuntimeError("El archivo ZIP no tiene la estructura interna de un XLSX.")

        # Segunda comprobación: openpyxl debe poder abrirlo.
        wb = load_workbook(ruta, read_only=True, data_only=False)
        wb.close()
    except RuntimeError:
        raise
    except Exception as e:
        raise RuntimeError(f"El XLSX no se puede abrir correctamente: {e}") from e


def descargar_excel_github(destino=CLOUD_PATH):
    """Descarga el Excel desde GitHub usando download_url/RAW y lo valida antes de usarlo.

    No se escribe sobre el archivo activo hasta que la descarga esté completa y validada.
    Esto evita que una respuesta truncada de la API deje un XLSX corrupto.
    """
    ...
    if r.status_code == 404:
        raise FileNotFoundError(
            ...
            "Sube primero un Excel .xlsx válido al repositorio."
        )
    r.raise_for_status()
    data = r.json()
    sha = data.get("sha")
    ...
    if not download_url:
        # Fallback RAW por si GitHub no entrega download_url.
        download_url = (
            f"https://raw.githubusercontent.com/"
            f"{GITHUB_OWNER}/{GITHUB_REPO}/{GITHUB_BRANCH}/{GITHUB_EXCEL}"
        )

    # Descargar a un archivo temporal; nunca pisamos el archivo bueno directamente.
    fd, tmp_name = tempfile.mkstemp(
        prefix="jc_excel_download_", suffix=".xlsx", dir=str(destino.parent)
    )
    os.close(fd)
    tmp = Path(tmp_name)
    ...
    try:
        ...
        rd = requests.get(download_url, headers=headers, timeout=60)
        rd.raise_for_status()
        tmp.write_bytes(rd.content)
...
def subir_excel_github(ruta, mensaje="Actualizar solicitudes desde Streamlit"):
    """Publica únicamente un XLSX ya validado en GitHub."""
    ruta = Path(ruta)
    _validar_xlsx(ruta)

    sha = _obtener_sha_github()
...
def guardar_excel_directo(ruta, df, mensaje="Actualizar solicitudes desde Streamlit"):
    """Actualiza SOLICITUDES en una copia temporal, valida el XLSX y recién después lo publica."""
    ruta = Path(ruta)
    ...
    with bloqueo_guardado(ruta):
        if not ruta.exists():
            raise FileNotFoundError("No existe el Excel local que se va a actualizar.")

        # Trabajamos sobre una copia para que un error nunca destruya el Excel activo.
        fd, tmp_name = tempfile.mkstemp(
            prefix="jc_excel_save_", suffix=".xlsx", dir=str(ruta.parent)
        )
        os.close(fd)
        tmp = Path(tmp_name)
        ...
        try:
            ...
            if "SOLICITUDES" in wb.sheetnames:
                ws = wb["SOLICITUDES"]
                # Limpiar únicamente el contenido de la hoja SOLICITUDES.
                # PROGRAMACION_RUTAS y las demás hojas permanecen intactas.
            ...
            for row_num, fila in enumerate(
                df[COLS].fillna("").itertuples(index=False), start=2
            ):
            ...
            # Validación final: si falla, NO se publica nada en GitHub.
            _validar_xlsx(tmp)

            # GitHub recibe solamente la copia validada.
            subir_excel_github(tmp, mensaje=mensaje)

            # Solo después de confirmar GitHub, actualizamos la copia local.
